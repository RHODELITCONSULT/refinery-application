 ...
 <script>
    $(document).ready(function() {
        const hasWarning = @json(Session::has('warning'));
        const hasInfo = @json(Session::has('info'));
        const hasSuccess = @json(Session::has('success'));
        const hasError = @json(Session::has('error'));

        if (hasWarning) {
            $('#solid-warningToast').toast('show');
        }
        if (hasInfo) {
            $('#solid-infoToast').toast('show');
        }
        if (hasSuccess) {
            $('#solid-successToast').toast('show');
        }
        if (hasError) {
            $('#solid-dangerToast').toast('show');
        }
    });
</script>
